...
def get_pretrained_loader(
    ...
    **model_kwargs: any,
    ...
    if "lama" in pretrained_model_name_or_path:  # Llama or llama
    ...
    elif "itnet" in pretrained_model_name_or_path:  # BitNet or bitnet
        return PretrainedBitnetLoader(
            pretrained_model_name_or_path=pretrained_model_name_or_path,
            huggingface_token=huggingface_token,
            **model_kwargs,
        )
        ...
        print(
            f"-> {pretrained_model_name_or_path} using default pretrained model loader"
        )
...
class PretrainedModelLoader:
    ...
    def load_tokenizer(self):
        ...
        try:
            ...
            print(
                "-> Error with `AutoTokenizer.from_pretrained(**self.loading_kwargs)`:",
                e,
            )
...
class PretrainedLlamaLoader(PretrainedModelLoader):
    def load(
        ...
        llama3_1 = (
            float(".".join(transformers.__version__.split(".")[:2])) > 4.42
        )  # 'Meta-Llama-3.1' in self.loading_kwargs['pretrained_model_name_or_path']
        if model_type is None:
            from transformers import LlamaForCausalLM as model_class

        elif "lolcats_llama_sharded" in model_type:
            from .modeling_llama_sharded import (
                ShardedLolcatsLlamaForCausalLM as model_class,
            )
...
class PretrainedBitnetLoader(PretrainedModelLoader):
    def load(
        self,
        model_type: str = None,
    ):
        if model_type is None:
            from transformers import BitNetForCausalLM as model_class
