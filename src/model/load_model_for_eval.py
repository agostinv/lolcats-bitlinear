...
def update_model_config_from_args(model_config, args):
    ...
    for arg in ["lk_skip_connection", "lk_zero_init"]:
        ...
        if argval is not None:
            setattr(
                model_config["attention"]["learned_kernel_kwargs"],
                arg[len("lk_") :],
                argval,
            )
            args.run_name += f"-{_format_arg(arg)}={argval}"
...
def get_lm_eval_model(
    model_kwargs: dict,  # model_loader.loading_kwargs
...
def load_model_from_checkpoint(
    ...
    if attn_mlp_checkpoint_path is not None:
        if len(attn_mlp_checkpoint_path.split("/")) == 4:
            ...
            model_config = (
                attn_mlp_checkpoint_path.split("/")[-1].split("-m=")[-1].split("-")[0]
            )
        model_config_path = join(config_dir, f"{model_config}.yaml")
        model_config = OmegaConf.load(model_config_path)
        ...
        if len(finetune_checkpoint_path.split("/")) == 4:
            ...
            model_config = (
                finetune_checkpoint_path.split("/")[-1].split("-m=")[-1].split("-")[0]
            )
        model_config_path = join(config_dir, f"{model_config}.yaml")
        model_config = OmegaConf.load(model_config_path)
    ...
    if lm_eval_model and attn_mlp_checkpoint_path is not None:
        lm = get_lm_eval_model(
            model_loader.loading_kwargs, path_to_lm_eval_harness, hedgehog_model=True
        )
        ...
        model = model_loader.load(
            model_type=model_config["attention"]["attention_type"]
        )
    try:
    ...
    if attn_mlp_checkpoint_path is not None:
        ...
        model = load_and_convert_attns(
            model, model_config, checkpoint_path=attn_mlp_checkpoint_path
        )[0]
