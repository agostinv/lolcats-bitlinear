"""
Subquadratic attention combining a small sliding window with topk + random masking + linear attention

For each layer:
- We first compute (softmax) attention over sliding windows and preserve topk keys
- We also add random keys to the window
...
def get_masks(
    ...
    device: torch.device,
    topk: int = None,
    random_topk: int = None,
    a_pred: torch.Tensor = None,
) -> tuple[torch.Tensor]:
    """
    Return masks for softmax and linear attention terms combining topk and random selection
    ...
    # If no topk/random_topk requested or no prediction scores provided, return default masks
    if (topk is None and random_topk is None) or a_pred is None:
    ...
    combined_topk_mask = torch.zeros(B, H, Q, K, dtype=torch.bool, device=device)

    # 1. Apply topk masking if requested
    if topk is not None:
        ap = a_pred.to(device)
        ...
        # Get top-k indices
        values, indices = torch.topk(ap_masked, k=topk, dim=-1)
        ...
        combined_topk_mask = combined_topk_mask | topk_mask

    # 2. Apply random masking if requested
    if random_topk is not None:
        # Build a random score tensor over eligible positions (causal & outside window & outside topk)
        rand_scores = torch.full((B, H, Q, K), float("-inf"), device=device)
        eligible = (~base_window) & base_causal & (~combined_topk_mask)
        rand_scores[eligible] = torch.rand_like(rand_scores[eligible])

        # Sample top-k indices based on the random scores
        values, indices = torch.topk(rand_scores, k=random_topk, dim=-1)

        # Build boolean mask for selected random positions
        random_mask = torch.zeros(B, H, Q, K, dtype=torch.bool, device=device)
        valid = values > float("-inf")
        random_mask.scatter_(-1, indices, valid)

        # Ensure random mask does not contain non-causal positions (safety)
        random_mask = random_mask & base_causal
        assert not (
            random_mask & upper_exp
        ).any(), "random_mask contains upper-triangular non-zero elements"

        combined_topk_mask = combined_topk_mask | random_mask

    # Combine base masks with combined topk/random mask. Cast back to int dtype to match callers
    new_window = (base_window | combined_topk_mask).to(torch.int)
    new_linear = (base_linear & (~combined_topk_mask)).to(torch.int)
...
def hybrid_attention_quadratic(
    ...
    """
    Hybrid attention combining sliding window and linear attentions with topk + random masking
    """
...
# ---------------------
class LolcatsLinearSlidingWindowTopkRandom(LolcatsLinearAttention):
    """
    Lolcats attention combining sliding window, topk, and random masking with linear attention
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.train_attention:
            ...
            # compute attn weights under sliding window with topk + random masking
...
class LinearAttentionSlidingWindowTopkRandomCache(LinearAttentionState):
    """
    ...
    def update_for_decoding(
        ...
        """
        Update the decoding KV and K states, and KV cache, during decoding
        """
        with torch.no_grad():
